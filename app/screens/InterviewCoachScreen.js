...
export default function InterviewCoachScreen({ navigation }) {
  ...
  const startSession = async () => {
    if (!role.trim()) {
      Alert.alert( isTurkish ? "Uyarı" : "Warning", isTurkish ? "Lütfen hedef pozisyonu gir." : "Please enter a target role." );
      return;
    }
    ...
    try {
      ...
      const data = await getInterviewQuestions({ role, level, mode, language, });
      ...
      Alert.alert( isTurkish ? "Hata" : "Error", isTurkish ? "Sorular alınırken bir sorun oluştu." : "Something went wrong while fetching questions." );
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGetFeedback = async () => {
    if (!currentAnswer.trim()) {
      Alert.alert( isTurkish ? "Uyarı" : "Warning", isTurkish ? "Lütfen cevap alanını doldur." : "Please write your answer first." );
      return;
    }
    ...
    try {
      const data = await getInterviewFeedback({ question: currentQuestion || "", answer: currentAnswer, language, });
      ...
      Alert.alert( isTurkish ? "Hata" : "Error", isTurkish ? "Geri bildirim alınırken bir sorun oluştu." : "Something went wrong while getting feedback." );
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNextQuestion = () => {
    if (currentIndex + 1 >= questions.length) {
      ...
      Alert.alert( isTurkish ? "Tebrikler" : "Nice work!", isTurkish ? "Oturumu tamamladın. İstersen yeni bir rolle tekrar deneyebilirsin." : "You’ve completed this session. You can start a new one with another role if you like." );
      return;
    }
  ...
  const currentQuestion = sessionActive && questions.length > 0 ? questions[currentIndex] : null;
  ...
  return (
    <View style={[ styles.optimizeContainer, { backgroundColor: theme.bg }, ]} >
      <ScrollView contentContainerStyle={styles.optimizeScroll} keyboardShouldPersistTaps="handled" >
        ...
        <TouchableOpacity style={[ styles.primaryButtonWide, { marginBottom: 16, marginTop: 4, backgroundColor: theme.accent, }, ]} onPress={startSession} >
        ...
        {currentQuestion && (
          <View style={[ styles.coachQuestionBox, { borderColor: theme.border, backgroundColor: theme.bgCard, }, ]} >
            <Text style={[ styles.coachProgressText, { color: theme.textSecondary }, ]} >
              {isTurkish ? "Soru" : "Question"} {currentIndex + 1} /{" "}
              {questions.length}
            </Text>
            <Text style={[ styles.coachQuestionText, { color: theme.textPrimary }, ]} >
              {currentQuestion}
            </Text>
        ...
        {feedback ? (
          <View style={[ styles.resultBox, { marginTop: 12, backgroundColor: theme.bgCard, borderColor: theme.border, }, ]} >
            ...
            <TouchableOpacity style={[ styles.secondaryButton, { marginTop: 10, borderColor: theme.border }, ]} onPress={handleNextQuestion} >
              <Text style={[ styles.secondaryButtonText, { color: theme.textPrimary }, ]} >
                {currentIndex + 1 >= questions.length  ? isTurkish ? "Oturumu Bitir" : "Finish Session" : isTurkish ? "Sonraki Soru" : "Next Question"}
              </Text>
            </TouchableOpacity>
          </View>
        ) : null}
